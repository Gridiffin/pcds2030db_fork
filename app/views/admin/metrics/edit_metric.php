...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['select_sector'])) {
    ...
    if ($sector_id > 0) {
        ...
        // Insert new outcome data only in sector_outcomes_data table (system no longer uses sector_metrics_data)
        $table_to_use = "sector_outcomes_data";
    ...
    if ($metric_id > 0) {
        ...
        if ($metric_data) {
            $data_json = $metric_data['data_json'];
              // Update table name in database (using sector_outcomes_data as per system notes)
        ...
        // Insert new metric (using sector_outcomes_data as per system notes)
