...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (filterForm) {
        ...
        // Set a flag to monitor if we're currently processing a search
        let isProcessingSearch = false;
        
        // Auto-submit functionality for text search
        const searchInput = filterForm.querySelector('input[type="text"]');
        if (searchInput) {
            searchInput.addEventListener('keyup', function() {
                // Only trigger search if not already processing and length criteria are met
                if (!isProcessingSearch && (this.value.length >= 3 || this.value.length === 0)) {
                    // Set processing flag
