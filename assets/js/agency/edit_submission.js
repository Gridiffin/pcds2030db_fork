...
function initEventListeners() {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (
      e.target.classList.contains("remove-target-btn") ||
      e.target.closest(".remove-target-btn")
    ) {
      e.preventDefault();
      e.stopPropagation();
      const button = e.target.classList.contains("remove-target-btn")
        ? e.target
        : e.target.closest(".remove-target-btn");
  ...
  document.addEventListener("click", function (e) {
    if (
      e.target.classList.contains("remove-attachment-btn") ||
      e.target.closest(".remove-attachment-btn")
    ) {
      e.preventDefault();
      const btn = e.target.classList.contains("remove-attachment-btn")
        ? e.target
        : e.target.closest(".remove-attachment-btn");
        ...
        fetch(`${window.APP_URL}/app/ajax/delete_program_attachment.php`, {
          ...
          .then((data) => {
            if (data.success) {
              ...
              const periodId = document.querySelector(
                'input[name="period_id"]'
              ).value;
              loadSubmissionData(periodId);
            } else {
              showToast(
  ...
  document.addEventListener("click", function (e) {
    if (
      ...
      e.target.form &&
      e.target.form.id === "submission-form"
    ) {
  ...
  fetch(`${window.APP_URL}/app/ajax/get_submission_by_period.php`, {
    ...
    .then((data) => {
      if (data.success) {
        ...
        showError(
          "Failed to load submission data: " + (data.error || "Unknown error")
        );
      }
    })
    .catch((error) => {
...
function updatePeriodStatusDisplay(periodId) {
  ...
  if (selectedOption && statusDisplay) {
    const hasSubmission =
      selectedOption.getAttribute("data-has-submission") === "true";
    ...
    if (status === "open") {
      ...
      statusHtml =
        '<span class="badge bg-secondary"><i class="fas fa-clock me-1"></i>Closed</span>';
    }

    if (hasSubmission) {
      const submissionStatus = selectedOption.textContent.includes("Draft")
        ? "Draft"
        : "Finalized";
      ...
      statusHtml +=
        ' <span class="badge bg-light text-dark ms-2"><i class="fas fa-plus me-1"></i>No Submission</span>';
    }
...
function showEditSubmissionForm(data) {
                        ...
                        <div class="flex-grow-1 description-section-card">
                            <div class="mb-4 h-100 d-flex flex-column">
                                ...
                                <textarea class="form-control flex-grow-1" id="description" name="description" rows="3"
                                          placeholder="Describe the submission for this period">${escapeHtml(
  ...
  const targetsContainerCard = document.querySelector(
    ".col-md-8 .card.shadow-sm"
  );
...
function showAddSubmissionForm() {
  ...
  fetchIncompleteTargets(periodId).then(() => {
    ...
    const periodName = selectedOption
                                        ...
                                        <h6 class="card-title mb-0">
                                            ...
                                            ${
                                              incompleteTargets.length > 0
                                                ? `<span class="badge bg-info ms-2">${incompleteTargets.length} auto-filled</span>`
                                                : ""
                                            }
                                        </h6>
                                    </div>
...
function autoFillIncompleteTargets() {
  ...
  if (incompleteTargets.length > 0) {
    showToast(
      "Info",
      `${incompleteTargets.length} incomplete targets from previous periods have been auto-filled. You can edit or remove them as needed.`,
      "info"
    );
  }
}
                    ...
                    <div class="row align-items-end">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" min="1" class="form-control form-control-sm target-counter-input" 
                                       name="target_counter[]" value="${escapeHtml(
                                         counter
                                       )}" placeholder="Counter (e.g., 1)">
                            </div>
                            <input type="hidden" name="target_number[]" value="${escapeHtml(
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea class="form-control" name="target_text[]" rows="2" required
                                      placeholder="Describe the target">${escapeHtml(
                                        target.target_text || ""
                                      )}</textarea>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select form-select-sm" name="target_status[]">
                                ...
                                <option value="in_progress" ${
                                  target.target_status === "in_progress"
                                    ? "selected"
                                    : ""
                                }>In Progress</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea class="form-control form-control-sm" name="target_remarks[]" rows="2"
                                      placeholder="Additional remarks for this target">${escapeHtml(
                                        target.remarks || ""
                                      )}</textarea>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-12">
                            ...
                            <textarea class="form-control form-control-sm" name="target_status_description[]" rows="2"
                                      placeholder="Provide details about achievements and current status">${escapeHtml(
...
function generateAttachmentsHtml(attachments) {
  ...
  attachments.forEach((attachment) => {
    const displayName =
      attachment.original_filename && attachment.original_filename.trim() !== ""
        ? attachment.original_filename
        : "Unnamed file";
    html += `
            <li class="mb-2 d-flex justify-content-between align-items-center attachment-item" data-attachment-id="${
              attachment.attachment_id
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        } • ${formatDate(attachment.upload_date)}</small>
                    </div>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <a href="${
                      window.APP_URL
                    }/app/ajax/download_program_attachment.php?id=${
      attachment.attachment_id
    }" 
                       class="btn btn-sm btn-outline-primary me-1" target="_blank" title="Download">
                        <i class="fas fa-download"></i>
                    </a>
                    <button type="button" class="btn btn-sm btn-outline-danger remove-attachment-btn" title="Remove" data-attachment-id="${
                      attachment.attachment_id
                    }">
...
function renderPendingFiles() {
  ...
  attachmentsList
    .querySelectorAll(".pending-attachment")
  ...
  const hasExisting =
    attachmentsList.querySelectorAll(".attachment-item").length > 0;
  ...
  pendingFiles.forEach((file, idx) => {
    ...
    listItem.className =
      "mb-2 d-flex justify-content-between align-items-center pending-attachment";
    listItem.innerHTML = `
            <div class="d-flex align-items-center">
                <i class="fas fa-file me-2 text-primary"></i>
                <div>
...
document.addEventListener("click", function (e) {
  if (
    e.target.classList.contains("remove-pending-file-btn") ||
    e.target.closest(".remove-pending-file-btn")
  ) {
    const btn = e.target.classList.contains("remove-pending-file-btn")
      ? e.target
      : e.target.closest(".remove-pending-file-btn");
        ...
        <div class="card-body">
                ...
                <div class="col-md-6">
                    ...
                    <textarea class="form-control" name="target_text[]" rows="2" required
                              placeholder="Describe the target">${
                                targetData
                                  ? escapeHtml(targetData.target_text || "")
                                  : ""
                              }</textarea>
                </div>
            </div>
            <div class="row mt-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select form-select-sm" name="target_status[]">
                        <option value="not_started" ${
                          targetData &&
                          targetData.target_status === "not_started"
                            ? "selected"
                            : ""
                        }>Not Started</option>
                        <option value="in_progress" ${
                          targetData &&
                          targetData.target_status === "in_progress"
                            ? "selected"
                            : ""
                        }>In Progress</option>
                        <option value="completed" ${
                          targetData && targetData.target_status === "completed"
                            ? "selected"
                            : ""
                        }>Completed</option>
                        <option value="delayed" ${
                          targetData && targetData.target_status === "delayed"
                            ? "selected"
                            : ""
                        }>Delayed</option>
                    </select>
                ...
                <div class="col-md-6">
                    <label class="form-label small">Remarks</label>
                    <textarea class="form-control form-control-sm" name="target_remarks[]" rows="2"
                              placeholder="Additional remarks for this target">${
                                targetData
                                  ? escapeHtml(targetData.remarks || "")
                                  : ""
                              }</textarea>
                </div>
            </div>
            <div class="row mt-3">
                <div class="col-md-12">
                    <label class="form-label small">Achievements/Status</label>
                    <textarea class="form-control form-control-sm" name="target_status_description[]" rows="2"
                              placeholder="Provide details about achievements and current status">${
                                targetData
                                  ? escapeHtml(
                                      targetData.status_description || ""
                                    )
                                  : ""
                              }</textarea>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control form-control-sm" name="target_start_date[]"
                           placeholder="Select start date" value="${
                             targetData ? targetData.start_date || "" : ""
                           }">
                </div>
                <div class="col-md-6">
                    <label class="form-label small">End Date</label>
                    <input type="date" class="form-control form-control-sm" name="target_end_date[]"
                           placeholder="Select end date" value="${
                             targetData ? targetData.end_date || "" : ""
                           }">
                </div>
            </div>
        </div>
...
function handleFormSubmission(form, submitter) {
  ...
  targetContainers.forEach((container) => {
    ...
    const statusDescElement = container.querySelector(
      '[name="target_status_description[]"]'
    );
    const remarksElement = container.querySelector('[name="target_remarks[]"]');
    const startDateElement = container.querySelector(
    ...
    if (!textElement || !counterElement) {
      console.warn(
        "Skipping target container with missing essential elements:",
        container
      );
      return;
    }
  ...
  fetch(`${window.APP_URL}/app/ajax/save_submission.php`, {
    ...
    .then((data) => {
      if (data.success) {
        ...
        showToast(
          "Error",
          data.error || "An error occurred while saving the submission.",
          "danger"
        );
      }
    })
    ...
    .finally(() => {
      submitButtons.forEach((btn) => {
        ...
        } else if (btn.name === "save_and_exit") {
          btn.innerHTML = '<i class="fas fa-save me-2"></i>Save and Exit';
        } else {
...
function refreshReportingPeriodsDropdown() {
  fetch(
    `${window.APP_URL}/app/ajax/get_reporting_periods.php?program_id=${window.programId}`
  )
    .then((response) => response.json())
    .then((data) => {
      if (data.success) {
        ...
        data.periods.forEach((period) => {
          ...
          periodSelector.innerHTML += `<option value="${
            period.period_id
          }" data-has-submission="${
            period.has_submission
          }" data-submission-id="${period.submission_id || ""}" data-status="${
            period.status
          }">${label}</option>`;
        });
...
function renderHistorySidebar(
  submissionId,
  targetId = "history-sidebar-container"
) {
  ...
  fetch(
    `${window.APP_URL}/app/ajax/get_submission_audit_history.php?submission_id=${submissionId}`
  )
    .then((response) => response.json())
    .then((data) => {
      ...
      fields.forEach((field) => {
        const label =
          history
            .find((entry) =>
              entry.field_changes.find((c) => c.field_name === field)
            )
            .field_changes.find((c) => c.field_name === field).field_label ||
          field;
    ...
    .then((data) => {
      ...
      let historyHtml = `
            <div class='history-header mb-3'>
                <div class='d-flex justify-content-between align-items-center'>
                    <h6 class='mb-0'><i class='fas fa-history'></i> Field History: ${getFieldDisplayName(
                      field
                    )}</h6>
                ...
                <small class='text-muted'>Showing ${data.history.length} of ${
        data.total_count || data.history.length
      } changes</small>
            </div>
        `;
      ...
      const sortedTargetKeys = Object.keys(groupedHistory).sort((a, b) => {
        ...
        const latestA = Math.max(
          ...targetA.map((entry) => new Date(entry.submitted_at).getTime())
        );
        const latestB = Math.max(
          ...targetB.map((entry) => new Date(entry.submitted_at).getTime())
        );
      ...
      limitedTargetKeys.forEach((targetKey) => {
        ...
        if (targetKey === "no_target") {
          ...
          if (index) {
            ...
            firstEntry.target_number &&
            firstEntry.target_number.trim() !== ""
          ) {
        ...
        historyHtml += `
                <div class='target-history-group mb-3'>
                    <div class='target-header d-flex align-items-center mb-2'>
                        ...
                        <small class='text-muted'>${entries.length} change${
          entries.length > 1 ? "s" : ""
        }</small>
                    </div>
          ...
          if (entry.change_type === "modified") {
            valueDisplay = `
                        <div class='change-values'>
                            <div class='old-value'><strong>From:</strong> ${
                              entry.old_value || "<em>empty</em>"
                            }</div>
                            <div class='new-value'><strong>To:</strong> ${
                              entry.new_value || "<em>empty</em>"
                            }</div>
                        </div>
                    `;
          } else if (entry.change_type === "added") {
            valueDisplay = `<div class='new-value'><strong>Added:</strong> ${
              entry.new_value || "<em>empty</em>"
            }</div>`;
          } else if (entry.change_type === "removed") {
            valueDisplay = `<div class='old-value'><strong>Removed:</strong> ${
              entry.old_value || "<em>empty</em>"
            }</div>`;
          }
                    ...
                    <div class='change-entry mb-2 p-2 border-start border-3 ${changeTypeClass}'>
                        <div class='change-header d-flex justify-content-between align-items-start'>
                            ...
                            <div class='change-meta text-end'>
                                <div class='change-date small'>${
                                  entry.formatted_date
                                }</div>
                                <div class='change-user small text-muted'>by ${
                                  entry.submitted_by
                                }</div>
                                ${
                                  entry.is_draft
                                    ? '<div class="draft-badge small text-warning">Draft</div>'
                                    : ""
                                }
                            </div>
                        </div>
      ...
      if (sortedTargetKeys.length > maxTargetGroups) {
        historyHtml += `
                <div class='text-center mt-3'>
                    <small class='text-muted'>+${
                      sortedTargetKeys.length - maxTargetGroups
                    } more targets</small>
                </div>
            `;
      }
