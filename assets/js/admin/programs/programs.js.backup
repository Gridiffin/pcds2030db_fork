/**
 * Admin Programs Page Script
 * 
 * This script handles all the client-side functionality for the admin programs page,
 * including filtering, sorting, and modal interactions for all three program tables:
...
document.addEventListener('DOMContentLoaded', () => {
    // Initialize all functionality for each of the three program sections
    ['draft', 'finalized', 'empty'].forEach(section => {
        initializeFilteringForSection(section);
        initializeSortingForSection(section);
    });

    // Initialize shared modals (delete and more actions)
    initializeDeleteModal();
    initializeMoreActionsModal();
});

/**
 * Sets up event listeners for the filter inputs for a specific section.
...
/**
 * Applies all active filters to the rows of a given table section.
...
function applyFilters(section) {
    ...
    // Add a "no results" row if necessary
    if (visibleCount === 0) {
        const colspan = table.querySelector('thead th').length;
 ...
 * @param {string} section - The section to initialize sorting for ('draft', 'finalized', or 'empty').
 */
...
/**
 * Sorts the rows of a table based on a column and direction.
 * @param {HTMLTableElement} table - The table element to sort.
 * @param {string} sortBy - The data attribute key to sort by ('name', 'agency', 'initiative', 'rating', 'date').
...
/**
 * Initializes the "More Actions" modal, creating it if it doesn't exist.
 */
 ...
 * @param {string} programType - The type of the program ('assigned' or 'created').
 */
...
/**
 * A helper function to trigger the delete modal from the "More Actions" modal.
