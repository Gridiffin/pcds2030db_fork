...
class ReportGenerator {
        ...
        try {
            ...
            const result = await response.json();
              if (result.success) {
                ...
                // Refresh recent reports table automatically
    ...
    async refreshRecentReports() {
        try {
            ...
            // Use the ReportAPI module if available
            if (typeof window.ReportAPI !== 'undefined' && window.ReportAPI.refreshReportsTable) {
                await window.ReportAPI.refreshReportsTable();
                console.log('Reports table refreshed successfully');
            } else {
                // Fallback: manual refresh
                console.log('ReportAPI not available, using fallback refresh');
                const reportsContainer = document.getElementById('recentReportsContainer');
                if (reportsContainer) {
                    const response = await fetch(`${this.config.appUrl || ''}/app/views/admin/ajax/recent_reports_table.php`);
                    if (response.ok) {
